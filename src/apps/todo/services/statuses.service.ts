...
export class StatusesService {
  ...
  private async checkAccess(id: number, user: User) {
    ...
    return (
      data.status &&
      data.status.project &&
      data.status.project.users.filter(eachUser => eachUser.id === user.id)
        .length > 0
    );
  }
  ...
  async findAll(
    ...
    try {
      ...
      if (options.q) {
        qb = qb.andWhere(
          '(status.title like :q or status.name like :q or status.id = :id)',
          {
            q: `%${options.q}%`,
            id: +options.q
          }
      ...
      options.sort =
        options.sort &&
        new Status().hasOwnProperty(options.sort.replace('-', ''))
          ? options.sort
          : '-id';
