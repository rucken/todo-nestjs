...
import { CORE_CONFIG_TOKEN, ICoreConfig, User } from '@todo-nestjs/core';
...
export class ProjectsService {
  ...
  async create(options: { item: Project }, user?: User) {
    ...
    try {
      options.item.statuses = await this.statusRepository.save(
  ...
  private async checkAccess(id: number, user: User) {
    ...
    return (
      data.project &&
      data.project.users.filter(eachUser => eachUser.id === user.id).length > 0
    );
  }
  ...
  async findAll(
    ...
    try {
      ...
      if (options.q) {
        qb = qb.andWhere(
          '(project.title like :q or project.description like :q or project.id = :id)',
          {
            q: `%${options.q}%`,
            id: +options.q
          }
